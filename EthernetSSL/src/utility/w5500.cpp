...
uint8_t W5500Class::write(uint16_t _addr, uint8_t _cb, uint8_t _data)
{
    ...
    uint8_t wdata;
    wdata = _data;
    gMIFInfo.WriteData(_addr, _cb, (const uint8_t*)&wdata, 1);
#else // USE_MS1000_MIF
    setSS();  
...
#if defined(ARDUINO_ARCH_AVR)
#if defined(USE_MS1000_MIF)
	gMIFInfo.WriteData(_addr, _cb, _buf, _len);
#else // USE_MS1000_MIF
    setSS();
    SPI.transfer(_addr >> 8);
    SPI.transfer(_addr & 0xFF);
...
#if defined(USE_MS1000_MIF)
	uint8_t _data = 0;
	gMIFInfo.ReadData(_addr, _cb, &_data, 1);
#else // USE_MS1000_MIF
    setSS();
    SPI.transfer(_addr >> 8);
    SPI.transfer(_addr & 0xFF);
...
#if defined(ARDUINO_ARCH_AVR)
#if defined(USE_MS1000_MIF)
	gMIFInfo.ReadData(_addr, _cb, _buf, _len);
#else // USE_MS1000_MIF
    setSS();
    SPI.transfer(_addr >> 8);
    SPI.transfer(_addr & 0xFF);
